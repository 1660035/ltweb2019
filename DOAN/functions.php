...
function removetest($userId2, $userId1, $stt) {
    // global $db;
    // $stmt = $db->prepare("DELETE FROM notifications WHERE userId1 = ? AND userId2 = ?");
    // $stmt->execute(array($userId1, $userId2));
    

    global $db;
    $stmt = $db->prepare("DELETE FROM  notifications WHERE stt= ? AND userId1 = ? AND userId2 = ? AND code = 2 OR code = 1");
    $stmt->execute(array($stt,$userId2, $userId1));
