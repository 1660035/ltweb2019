...
function createUser($displayName, $email, $hashPassword)   {
    ...
    $id = $db->lastInsertId();
    sendEmail($email, $displayName, 'Kich hoat tai khoan', "Ma kich hoat la <a href=\"$BASE_URL/activate.php?code=$code\">$BASE_URL/BTCN08/activate.php?code=$code</a>");
...
function forgotUser($email)   {
    ...
    \">$BASE_URL/BTCN08/reset.php?email=$email]</a> ");
