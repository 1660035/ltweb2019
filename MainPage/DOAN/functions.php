...
function newUser($displayName, $email, $hashPassword, $phone, $ddmmyyyy)  {
    ...
    $stmt = $db->prepare("INSERT INTO users(displayName, email, password, phone, ddmmyyyy, status, code) VALUE(?,?,?,?,?,?,?)");
    $stmt->execute(array($displayName, $email, $hashPassword, $phone, $ddmmyyyy, 0, $code));
...
function getListFriend($id)    {
    global $db;
    $stmt = $db->prepare(" SELECT xx.*, u.avatar, u.displayName
